...
public class ClockPane extends Pane {
    ...
    HBox hButtons = new HBox(playPause);
    ...
    public ClockPane(String zone) {
        if(zone.equals("JST")){
            setPrefHeight(h);
            setPrefWidth(w);
            timeline = new Timeline(new KeyFrame(Duration.seconds(1), e -> jUpdate()));
            timeline.setCycleCount(Timeline.INDEFINITE);
            setJSTTime();
            playPause.setOnAction(e -> {
                if (timeline.getStatus() == Animation.Status.RUNNING) {
        ...
        if(zone.equals("GMT")){
            setPrefHeight(h);
            setPrefWidth(w);
            timeline = new Timeline(new KeyFrame(Duration.seconds(1), e -> gUpdate()));
            timeline.setCycleCount(Timeline.INDEFINITE);
            setGMTTime();
            playPause.setOnAction(e -> {
                if (timeline.getStatus() == Animation.Status.RUNNING) {
    ...
    public void setJSTTime() {
        Calendar calendar = new GregorianCalendar();
        
        this.hour = calendar.get(Calendar.HOUR_OF_DAY) - 7;
        this.minute = calendar.get(Calendar.MINUTE);
        this.second = calendar.get(Calendar.SECOND);
